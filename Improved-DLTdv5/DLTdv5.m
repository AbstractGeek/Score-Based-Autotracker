...
% Version 6 - Dinesh Natesan 05/01/16 [Added a better template search
% function.]
...
%% case 99 - GUI figure creation
  case {99} % Initialize the GUI
    ...
    h(38) = uicontrol('Parent',h(1),... % Autotrack search area size
      'Units','characters','BackgroundColor',[1 1 1],'Position',...
      [36.8 top-24.13 9.8 1.6],'String','30','Style','edit','Tag',...
...
%             double(imdat),gammaS,h);
          [xnew,ynew,fitt]=regionTM(double(roibase),[x,y], ...
            double(imdat),gammaS,h);
        catch
          fitt=-1;
          xnew=NaN;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% regionTM
function [x,y,fit]=regionTM(prevRegion,prevXY,newData,gammaS,h)
% function [x,y,fit]=regionTM(prevRegion,prevXY,newData,gammaS,h)
%
% A Euclidean distance based generalized image tracker.
%
% Version history:
% Basic Template from 4.0 - Ty Hedrick, 12/10/2005
% Version 1.0 - Dinesh Natesan, 05/04/2016

xy=prevXY; % base level xy
ssize=floor(size(prevRegion,1)/2); % search area deviation
isize = [size(newData,2),size(newData,1)];
...
for i=1:size(chunk,3)
  image = chunk(:,:,i);
  tsize = iff(rem(ssize,2),ssize+1,ssize);  
  template = get_track_template(prevRegion(:,:,i),...
      (size(prevRegion(:,:,i))+1)./2,[tsize,tsize],size(prevRegion(:,:,i)));
  templateSize = size(template);

  weight = gaussian2D(templateSize,(templateSize+1)/2,round((templateSize-1)/(2*2.0)));
  [J(i),I(i),fit(i),scores] = simpleTemplateMatcher(image,template,weight);
  
  % do sub-pixel interpolation
  [I(i),J(i)] = subPixPos(scores,I(i)-tsize/2+1,J(i)-tsize/2+1);

  % Fix pixels (just a hack for now - will create a function for
  % templatematcher later).
  I(i) = I(i)+tsize/2-1;
  J(i) = J(i)+tsize/2-1;
...
% place in world coordinates
x=xy(1)+J(fit==max(fit))-ssize-1;
y=xy(2)+I(fit==max(fit))-ssize-1;
...
%% Custom functions for regionTM
% Added by Dinesh Natesan, 1st May 2016
...
scores = scores.*6; % rescaling for easy setting of DLTdv5 threshold
